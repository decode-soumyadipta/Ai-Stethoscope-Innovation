...
@app.route('/mfcc_image')
def mfcc_image():
    """Serve the latest MFCC plot image."""
    mfcc_path = os.path.join('static', 'latest_mfcc.png')
    if not os.path.exists(mfcc_path):
        # Return a placeholder or 404 if not found
        from flask import abort
        return abort(404)
    return send_file(mfcc_path, mimetype='image/png')
