...
N_MELS = 128
N_MFCC = 20
LATEST_MFCC_PATH = 'dashboard/static/latest_mfcc.png'
...
def preprocess_audio(audio_data, sr=SAMPLE_RATE):
    """
    Preprocess decoded audio data (int16 array) into a stacked mel-spectrogram + MFCC feature array.
    Also generates and saves an MFCC plot for visualization.
    """
    ...
    # Mel-spectrogram
    S = librosa.feature.melspectrogram(y=waveform, sr=sr, n_fft=2048, hop_length=512, n_mels=N_MELS)
    ...
    # Save MFCC plot for dashboard
    try:
        plt.figure(figsize=(8, 3))
        librosa.display.specshow(mfcc, x_axis='time')
        plt.colorbar(format='%+2.0f dB')
        plt.title('MFCC (latest audio)')
        plt.tight_layout()
        plt.savefig(LATEST_MFCC_PATH)
        plt.close()
    except Exception as e:
        print(f"Error saving MFCC plot: {e}")
